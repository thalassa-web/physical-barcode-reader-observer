 ...
 */
const AIMPrefixes = new Map<string, AIMSymbology>([
...
export const getAIMSymbology = (value: string) : {value: string, aim: AIMSymbology} => {
    if (value.startsWith(']')) {
        for (let entry of Array.from(AIMPrefixes.entries())) {
...
const isEan = (value: string, length: 8 | 13): boolean => {
    ...
    return (aimSymbology.aim === AIMSymbology.UNKNOWN && isOnlyDigits(value, length) && eanKeyController(value))
...
const isUpc = (value: string): boolean => {
    const aimSymbology = getAIMSymbology(value);
    return (aimSymbology.aim === AIMSymbology.UNKNOWN && isOnlyDigits(value, 12) && eanKeyController(`0${value}`))
